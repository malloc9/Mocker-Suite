**Objective:** Set up a monorepo structure with separate frontend and backend applications to simplify dependency management and code sharing.

**Tasks:**
- [ ] Create project root directory and initialize with `pnpm init` (set `private`: true)
- [ ] Configure pnpm Workspaces by creating `pnpm-workspace.yaml` with `packages: - "apps/*"`
